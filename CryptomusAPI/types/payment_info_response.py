...
class PaymentInfoResponse:
    ...
    def from_json(cls, data: dict[str, Any]) -> PaymentInfoResponse:
        ...
        convert_from = ConvertInfo.from_json(convert_data) if convert_data else None
        return cls(state=data["state"], from_=from_, convert_from=convert_from, **data["result"])